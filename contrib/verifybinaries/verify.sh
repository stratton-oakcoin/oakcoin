...
###   This script attempts to download the signature file SHA256SUMS.asc from oakcoin.org
...
WORKINGDIR="/tmp/oakcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://oakcoin.org/bin/"
VERSIONPREFIX="oakcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'oakcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Oakcoin Core binary release signing key installed?"
   fi
