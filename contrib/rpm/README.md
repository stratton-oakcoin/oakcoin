...
The RPM spec file provided here is for Oakcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://oakcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Oakcoin git repository](https://github.com/oakcoin/oakcoin) but are not part of
...
Currently only `Source10` is used, for the example `oakcoin.conf` file.
...
in the oakcoin git repository. At present only an SVG version of the Oakcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the oakcoin
...
needed when 0.12.1 is released, a proper fix is already in the Oakcoin git
master branch.
...
idea to build Oakcoin Core against a static version of that library compiled
...
The `oakcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `oakcoin-server` package. At this point, most RPM based
...
The `oakcoin-server` package should have SELinux support. How to properly do
...
Oakcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/oakcoin/). He has been
packaging Oakcoin for Fedora at least since 2012.
...
4. I renamed the `oakcoin` package that contains the Qt GUI to `oakcoin-core` as
that appears to be how the general population refers to it, in contrast to
`oakcoin-xt` or `oakcoin-classic`. I wanted to make sure the general population
