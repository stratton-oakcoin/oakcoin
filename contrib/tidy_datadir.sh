#!/bin/bash
# Copyright (c) 2013 The Oakcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Oakcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Oakcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Oakcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Oakcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Oakcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Oakcoin 0.8 datadir."
    ;;
esac
