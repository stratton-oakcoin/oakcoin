...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = COakcoinAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-oakcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(COakcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(COakcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = COakcoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(COakcoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(COakcoinAddress(address).ToString());
}
 ...
 * https://oakcointalk.org/?topic=54527, or
 * https://github.com/oakcoin/oakcoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(COakcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(COakcoinAddress(dest).ToString(), key);
}
